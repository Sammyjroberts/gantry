...
import (
	...
	"github.com/Sammyjroberts/gantry/libs/go/segments"
	"github.com/Sammyjroberts/gantry/libs/go/stream"
)

// Persistence is Edge's durable telemetry tier, assembled from the shared libs:
...
// the segment glob. It is a thin assembly the Edge server wires in; Backend
...
func NewPersistence(ctx context.Context, storeDir string, db *sql.DB, bus *stream.Bus) (*Persistence, error) {
	...
	engine, err := duckdb.New(duckdb.DefaultProvider(storeDir), duckdb.Config{
		...
		CatalogPath:  filepath.Join(storeDir, "edge.db"),
	})
	if err == nil {
		p.SQL = engine
	}
