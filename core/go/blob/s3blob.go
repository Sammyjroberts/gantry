...
// Cloud's blob store. Keys map directly to object names within one bucket
// (the "/"-separated logical key IS the object key), so the same
// "segments/<device>/<start>-<end>.parquet" layout Bench writes on disk becomes
// the object key in the cloud, and Bench→Cloud sync is a plain object upload.
type S3 struct {
	client *minio.Client
	bucket string
}
