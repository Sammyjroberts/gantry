...
// and builds an MCP server whose implementation name is "gantry-core". Bench
// mounts it today over the same HTTP port it already serves; Cloud will mount
...
// high-water mark. Bench supplies one via BusStreamStater.
...
// so the mcp package keeps no dependency on core/go/experiments; that package's
