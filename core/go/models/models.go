...
// video, it runs identically on Bench (fs blobs) and core (s3 blobs); only the
...
// core/go/blob.Store, the same shared object-storage seam the video service
// uses. See core/go/video/blob.go for why this is an alias rather than a
