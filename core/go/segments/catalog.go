...
// Catalog is the segment index behind an interface: SQLite on Bench today
// (SQLCatalog), Postgres on Cloud later. Add commits a flushed segment and
...
// SQLCatalog implements Catalog over the Bench SQLite database (the segments +
// segment_state tables from migration 0002). It shares the *sql.DB with the rest
// of Bench; benchdb caps the pool at one connection so writes never contend.
