...
// (embedded) NATS server for Edge, or by connecting to an external NATS URL for
// Backend. Both paths expose the same publish/subscribe surface.
type Bus struct {
	nc  *nats.Conn
...
// connection (no TCP socket is opened). This is how Edge runs.
...
// Connect dials an external NATS server (the Backend path). The remote server
