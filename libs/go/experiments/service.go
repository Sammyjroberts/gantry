// Package experiments implements experiment tracking for Edge: named time ranges
...
// used by the CSV export path. Metadata is persisted in the Edge SQLite store
// (libs/go/edgedb); frames are never copied — an experiment only indexes a range
