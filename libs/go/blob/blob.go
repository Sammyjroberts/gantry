...
// interface shared by Edge and Backend so the segment store (and anything else
// that persists immutable files) is written once against one seam. Edge wires
// the filesystem implementation (fsblob, rooted under <data-dir>/blobs);
// Backend wires the S3-compatible implementation (s3blob, against MinIO locally
...
// object through Get). s3blob does not satisfy it; on Backend the equivalent is
