...
//!
//! Run it with:
//!     cargo run -p gantry-edge --example simulator
//!     GANTRY_ENDPOINT=http://localhost:4780 cargo run -p gantry-edge --example simulator
...
use gantry_edge::{ChannelSpec, Client};
...
fn main() {
    ...
    while running.load(Ordering::SeqCst) {
        ...
        client.send_bool("status.armed", (t as u64 / 5).is_multiple_of(2));

        tick += 1;
        if tick.is_multiple_of(HZ * 2) {
            let s = client.stats();
            println!(
