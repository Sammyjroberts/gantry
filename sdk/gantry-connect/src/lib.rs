//! # Gantry Connect — SDK core
//!
//! An OTEL-like telemetry SDK for robot/rocket firmware and bench tools. You embed it,
//! push samples on named channels, and it batches frames and ships them to a Gantry Edge
//! or Backend ingest endpoint.
//!
...
//! `gantry-connect-agent` (a collector daemon), not the in-process SDK.
//!
//! ## Example
//! ```no_run
//! use gantry_connect::{Client, ChannelSpec};
//! use std::time::Duration;
//! # struct T;
//! # impl gantry_connect::Transport for T {
//! #   fn publish(&self, _b: gantry_connect::FrameBatch) -> Result<u64, gantry_connect::TransportError> { Ok(0) }
//! # }
//! # let transport = T;
...
// Re-export the wire types so users need only depend on `gantry-connect`.
pub use gantry_connect_proto::prost;
pub use gantry_connect_proto::v1::{value, ChannelInfo, Frame, FrameBatch, Value, ValueKind};
